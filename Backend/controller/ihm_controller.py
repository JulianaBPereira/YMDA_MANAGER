...
from Backend.services import funcionarios_service, modelos_service, producao_service
from Backend.models import Funcionario, Modelo
from Backend.websocket_manager import enviar_atualizacao_dashboard, enviar_atualizacao_registros
...
def validar_rfid() -> Union[Response, Tuple[Response, int]]:
    try:
        ...
        from Backend.services import tags_temporarias_service
...
def listar_operacoes_ihm() -> Union[Response, Tuple[Response, int]]:
    try:
        from Backend.services import operacao_service
...
def listar_postos_ihm() -> Union[Response, Tuple[Response, int]]:
    try:
        from Backend.services import posto_service
...
def registrar_producao_ihm() -> Union[Response, Tuple[Response, int]]:
    try:
        ...
        print(f"[IHM] Notificando WebSocket após registrar produção...")
        enviar_atualizacao_dashboard()
        enviar_atualizacao_registros()
...
@ihm_bp.route('/contexto-operacao', methods=['GET'])
def buscar_contexto_operacao() -> Union[Response, Tuple[Response, int]]:
    """Endpoint único que retorna todo contexto necessário para a página de operação"""
    try:
        from Backend.services import operacao_service
        
        nome_funcionario = request.args.get('funcionario')
        resultado = operacao_service.buscar_contexto_ihm(nome_funcionario)
        
        return jsonify(resultado)
        
    except Exception as e:
        import traceback
        print(f"[IHM] Erro ao buscar contexto: {str(e)}")
        print(traceback.format_exc())
        return jsonify({
            "status": "error",
            "message": str(e)
