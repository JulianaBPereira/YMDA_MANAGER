...
class DispositivoRaspberry:
    ...
    def save(self) -> 'DispositivoRaspberry':
        ...
        if self.dispositivo_id:
            ...
            query = """
                ...
                WHERE id = %s
            """
            params: Tuple[Any, ...] = (self.serial, self.nome, self.dispositivo_id)
            DatabaseConnection.execute_query(query, params)
        else:
            # Inserir com RETURNING id para PostgreSQL
            query = """
                INSERT INTO dispositivos_raspberry (serial, nome, data_registro) 
                VALUES (%s, %s, CURRENT_TIMESTAMP) 
                RETURNING id
            """
            params = (self.serial, self.nome)
    ...
    def buscar_por_id(dispositivo_id: int) -> Optional['DispositivoRaspberry']:
        ...
        query = """
            SELECT id, serial, nome, data_registro 
    ...
    @staticmethod
    def criar(serial: str, nome: Optional[str] = None) -> 'DispositivoRaspberry':
        """Método estático para criar um novo dispositivo"""
        dispositivo = DispositivoRaspberry(serial=serial, nome=nome)
