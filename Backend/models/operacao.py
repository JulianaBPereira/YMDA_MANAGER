...
class Operacao:
    ...
    @staticmethod
    def listar_contexto_ihm() -> List[Dict[str, Any]]:
        """Busca todas as operações com contexto completo em uma única query"""
        query = """
            SELECT 
                o.operacao_id,
                COALESCE(o.nome, o.codigo_operacao) as operacao,
                o.codigo_operacao,
                pr.nome as produto,
                m.modelo_id,
                m.nome as modelo_descricao,
                p.nome as posto,
                COALESCE(
                    (SELECT array_agg(DISTINCT pe.nome) 
                     FROM operacao_pecas op2 
                     JOIN pecas pe ON op2.peca_id = pe.peca_id 
                ...
                COALESCE(
                    (SELECT array_agg(DISTINCT toten_nome) 
                     FROM operacao_totens 
                     WHERE operacao_id = o.operacao_id),
                    ARRAY[]::TEXT[]
                ) as totens
            FROM operacoes o
            LEFT JOIN produtos pr ON o.produto_id = pr.produto_id
            LEFT JOIN modelos m ON o.modelo_id = m.modelo_id
            LEFT JOIN postos p ON o.posto_id = p.posto_id
            ORDER BY o.nome, o.codigo_operacao
        """
        ...
        operacoes = []
        operacoes_vistas = set()
        
        for row in resultados:
            codigo = row[2]
            if codigo in operacoes_vistas:
                continue
            operacoes_vistas.add(codigo)
            
            pecas_nomes = row[7] if row[7] else []
            pecas_codigos = row[8] if row[8] else []
            
            pecas = []
            for i in range(max(len(pecas_nomes), len(pecas_codigos))):
                pecas.append({
                    'nome': pecas_nomes[i] if i < len(pecas_nomes) else '',
                    'codigo': pecas_codigos[i] if i < len(pecas_codigos) else ''
                })
            
            operacoes.append({
                'id': row[0],
                'codigo': codigo,
                'nome': row[1] or codigo,
                'produto': row[3] or '',
                'modelo': {
                    'id': row[4],
                    'codigo': row[5] or '',
                ...
                'posto': row[6] or '',
                'pecas': pecas,
                'codigos': pecas_codigos,
                'totens': row[9] if row[9] else []
            })
